...
def type_recovery(type_addr):
    ...
    name_len = getByte(s_type.add(name_off + len_off))
    ...
    if name_len < 0:
        name_len = getBytes(s_type.add(name_off + len_off), 2)
        for b in name_len:
            print(hex(b))
    ...
    if name is not None:
        print(name.getValue())
    if name is None:
        name_addr_hack = name_addr.add(1) #this is very hacky and bad
        name = getDataAt(name_addr_hack)
        if name is None:
            try:
                print("name_addr: ", name_addr_hack, "name_len: ", name_len, \
                    "name_len_addr", s_type.add(name_off + len_off))
                remove_data(name_addr, name_len)
                name = createAsciiString(name_addr_hack, name_len)

            except:
                name_len = getBytes(s_type.add(name_off + len_off), 2)
                name_len = (name_len[0] & 0xff) | name_len[1] & 0xff
                print("name_addr: ", name_addr_hack, "name_len: ", hex(name_len), \
                    "name_len_addr", s_type.add(name_off + len_off))
                remove_data(name_addr, name_len)
                try:
                    name = createAsciiString(name_addr_hack, name_len)
                except:
                    print("name_addr: ", name_addr, "name_len: ", name_len, \
                        "name_len_addr", s_type.add(name_off + len_off))
                    remove_data(name_addr, name_len)
                    name = createAsciiString(name_addr, name_len)
    if tflag_ex_star:
    ...
    if SymbolUtilities.containsInvalidChars(name_type):
        name_type = SymbolUtilities.replaceInvalidChars(name_type, True)
    ...
    if kind == 0x13:
        ...
        for i in range(out_count):
            output = getAddressAt(type_addr.add(4*ptr_size+8+8+ptr_size +tflag_uncomm*16 +in_count*ptr_size + i*ptr_size))
            type_recovery(output)
            outputs.append(getSymbolAt(output).getName())
        if last_param == 0x80 and len(params) > 0:
            params[-1] = params[-1].replace("[]","...")
        setPreComment(type_addr,"func(" + ", ".join(params) + ")" + " (" +  ", ".join(outputs) + ")")
    ...
    if kind == 0x16:
        new_address =currentProgram.getAddressFactory().getAddress(hex(getInt(type_addr.add(4*ptr_size+8+8)))) 
        type_recovery(new_address)
    ...
    if kind == 0x19:
        struct_field =getAddressAt(type_addr.add(5*ptr_size+8+8))
        fields = []
        fields_length = getInt(type_addr.add(6*ptr_size+8+8))
        for i in range(fields_length):
            struct_field_name = getAddressAt(struct_field)
            name_length_address = struct_field_name.add(len_off)
            name_length = getByte(name_length_address)
            name_address = getAddressAt(struct_field).add(len_off+1)
            remove_data(name_address, name_length)
            ...
            new_type = getAddressAt(struct_field.add(ptr_size))
            print "new_type: 0x%x" % new_type.getOffset()
            type_recovery(new_type)
            struct_field = struct_field.add(3*ptr_size)
            fields.append(name.getValue()  + " " + getSymbolAt(new_type).getName())
        setPreComment(type_addr,"type " + name_type + " struct{" + "\n\t" + "\n\t".join(fields) + "\n" + "}")
...
len_off = 1 #this should be determined via a function but for 1.17+ this is the magic value
